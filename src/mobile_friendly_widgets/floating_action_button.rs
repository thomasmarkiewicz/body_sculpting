...
pub struct FloatingActionButton<T> {
    label: Label<T>,
    label_size: Size,
    size: Size,
}

impl<T: Data> FloatingActionButton<T> {
    ...
    /// });
    /// ```
    pub fn new(text: impl Into<LabelText<T>>) -> FloatingActionButton<T> {
        FloatingActionButton {
            label: Label::new(text).with_text_size(32.0),
...
impl<T: Data> Widget<T> for FloatingActionButton<T> {
    ...
    fn layout(
        ...
        bc.debug_check("FloatingActionButton");
